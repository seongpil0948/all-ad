...
export class TeamService {
  ...
  async inviteTeamMember(email: string, role: UserRole) {
    try {
      const {
        data: { user },
      ...
      const { data: masterTeam } = await this.supabase
        .from("teams")
        .select("id")
      ...
      const { data: edgeFunctionResponse, error: edgeFunctionError } =
        await this.supabase.functions.invoke("resend", {
          body: {
            to: email,
  ...
  async updateTeamMemberRole(memberId: string, role: UserRole) {
    try {
      const {
        data: { user },
      } = await this.supabase.auth.getUser();
  ...
  async createTeamForUser() {
    try {
      ...
      const { data: newTeamId, error } = await this.supabase.rpc(
