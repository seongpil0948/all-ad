...
export async function GET(request: NextRequest) {
  try {
    ...
    // 인증 코드 확인
    if (!code) {
      log.error("Google Ads OAuth callback missing authorization code");
    ...
    if (state) {
      ...
      log.info("Google Ads OAuth state received", { state });
    }
    ...
    log.info("Google Ads OAuth callback successful", {
    ...
    log.error("Google Ads OAuth callback error", { error });
