...
serve(async (req: Request) => {
  ...
  try {
    console.log("Starting Google Ads full sync (daily cron)");
    ...
    console.log(`FULL sync target accounts: ${accounts?.length || 0}`);

    const results = [];

    // Process each Google Ads account with full sync
    for (const account of accounts || []) {
      try {
        ...
        // Calculate date range for full sync (last 30 days)
        const endDate = new Date();
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - 30);

        // Call platform-sync endpoint with full sync
        ...
        results.push({
          ...
          metrics_synced: syncResult.metrics_count || 0,
        });

        console.log(`Full sync completed: ${account.account_name}`, {
          accountId: account.customer_id,
          syncType: "FULL",
          campaigns: syncResult.campaigns_count,
          metrics: syncResult.metrics_count,
        });
      } catch (error) {
    ...
    console.error("Google Ads full sync cron job failed:", error);
