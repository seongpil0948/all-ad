...
export function getRecordDir(testInfo: TestInfo) {
  ...
  const path = `test-records/${testInfo.titlePath.slice(1).join("/")}`;
...
base.afterEach(async ({ page }, testInfo) => {
  if (!testConfig.saveRecord) return;
  const env = testInfo.project.name;
  await page.screenshot({
    path: `${getRecordDir(testInfo)}/after-${env}.jpeg`,
    type: "jpeg",
    scale: "css",
  });
...
interface IMyFixtures {
  /**
   * Add an annotation to the test for categorization and reporting
   * @param type Type of annotation from AnnotationType enum
   * @param description Description or value for the annotation
   */
  pushAnnotation(type: AnnotationType, description: string): void;

  /**
   * Take a screenshot before the test execution
   * The screenshot will be saved in the test records directory
   */
...
export const test = base.extend<IMyFixtures>({
  ...
  screenshotBefore: async ({ page }, use, testInfo) => {
    await use(async () => {
      if (!testConfig.saveRecord) return;
      await page.screenshot({
        path: `${getRecordDir(testInfo)}/before-${testInfo.project.name}.jpeg`,
        type: "jpeg",
        scale: "css",
      });
    });
